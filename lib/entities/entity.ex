  ...
  defmacro __using__(_) do
    quote do
      ...
      defp execute_command(
             command,
             %EntityState{:id => id, :state => state, :behavior => behavior} = entity_state
           )
        ...
        case behavior.execute(%ExecutionContext{id: id, state: state}, command) do
          ...
          _ ->
            {nil, %EntityState{id: id, state: state, behavior: behavior}, :infinity}
        end
      end

      defp apply_events(events, %EntityState{:id => _id, :state => state, :behavior => behavior})
